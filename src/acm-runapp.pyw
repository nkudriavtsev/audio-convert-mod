...
class acmApp(interface.Controller):
  ...
  def checkFormats(self):
    ...
    self.ui.main2FormatCombobox.remove_all()
    for format in list(formats.FORMATS.values()):
      if format.get()[0] == True:
        self.ui.main2FormatCombobox.append_text(format.__class__.__name__.upper())
    if active >= 0:
  ...
  def on_main2FormatCombobox_changed(self, widget):
    ...
    self.ui.main2ExtensionCombobox.remove_all()
    ...
    for i in formats.FORMATS[formatValue.lower()].extensions:
      self.ui.main2ExtensionCombobox.append_text('.%s' % i)
  ...
  def startConversion(self):
    ...
    for i in files:
      ...
      if not self.encodeonly:
        ...
        if not inputFormatObject.decoderHasProgress(): # a52dec and opusdec doesn't support progress
          self.fraction = -1
      ...
      if not (os.path.exists(newname) and self.ui.main2ExistsRadio3.get_active()):
        ...
        if not self.decodeonly:
          ...
          self.logger.logmsg("DEBUG", _("Executing command: %s") % command)
          self.processId = sub.pid
          if not outputFormatObject.encoderHasProgress(): # ffmpeg doesn't support progress
            self.fraction = -1
            progressBar.startPulse()
            while sub.poll() == None:
              while Gtk.events_pending():
